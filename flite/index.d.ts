 ...
 */
interface IFLITEConfiguration extends IPluginConfig<IFLITETooltipOptions, IFLITECommand> {
	/**
	 ...
	 */
	readonly onEditorDestroy: EditorDestroyPolicy;
	 ...
	 *	 
	 */
	readonly tooltips: IFLITETooltipOptions,
	 ...
	 */
	readonly contextMenu: ReadonlyArray<string>;
	 ...
	 */
	readonly ignoreCommands: ReadonlyArray<string>;
	 ...
	 */
	readonly ignoreSelectors: string | string[];
	 ...
	 */
	readonly opaqueSelectors: string | string[];
	 ...
	 */
	readonly deletableSelectors: string;

	/**
	 * Initial user to track with
	 */
	readonly user?: PartialWith<IFLITEUser, "id">;

	readonly users: PartialWith<IFLITEUser, "id">[];

	/**
	 * @member FLITE.Configuration
	 * @property {Boolean} acceptRejectInReadOnly
	 */
	readonly acceptRejectInReadOnly: boolean;
	/**
	 ...
	 * Legal values:
	 * 
	 * - `true`: the command to stop tracking will not require resolution of all changes
	 * - `false`: The user can't toggle tracking off if there are pending changes.
	 * - `"accept"`:  Accept all changes before turning tracking off
	 * - `"accept"`: Reject all changes before turning tracking off
	 * - A function that returns either one of the previous values, or a promise that resolves to one of these values.
	 * 
	 * if a function is provided, it is invoked with the plugin instance as a parameter. FLITE will act according to
	 * the returned value.
	 * 
	 */
	 ...
	 */
	readonly ignoreContainers: string | Array<string> | boolean;
	 ...
	 */
	readonly removeDeleteOnPaste: boolean;
	 ...
	 */
	readonly maxDocumentReadyWaitSeconds?: number;
	 ...
	 */
	readonly spellCheckerClasses?: string | string[];
	 ...
	 * A collection of classes to add to the plugin's spell checker css selector
	 */
	readonly trackApiInsert: boolean;
	...
	*/
	 ...
	 */
	readonly allowFindDeletedText: boolean;
	 ...
	 *
	 */
	readonly copyBehavior: FLITECopyBehavior;
	 ...
	 *
	 */
	readonly sessionPolicy: FLITESessionPolicy;
	 ...
	 *
	 */
	readonly readonlyInsertPolicy: PassiveInsertMode;
	 ...
	 */
	readonly nonTrackingCommands: string | string[];

	/**
	 * If true, track list items converted from bullet -> number or vice versa as added
	 */
	readonly trackListTypeChange: boolean;
	 ...
	 */
	readonly passiveTracking: boolean;

	readonly statusCallback?: FLITEStatusCallback;

	readonly clipboardSupport: boolean;
	 ...
	 * Defaults to false
	 */
	readonly trackIME: boolean;

	readonly readonlyDelete: boolean;

	/**
	 * Defaults to "none"
	 */
	readonly imeMixedTrackingPolicy: IMEMixedPolicy;

	readonly orphanContainer: FLITEOrphanPolicy;

	/**
	 * @property {Boolean | "delete"} [disableSpellcheck=false]
	 * @since FLITE 1.6.25
	 * Some browser spellcheckers honor the `spellcheck` attribute of the elements. If you want to prevent the browser from
	 ...
	 */
	readonly disableSpellcheck: SpellcheckAttributePolicy;
	...
	/**
	 * How should FLITE treat table content, when part of the table is deleted (either accepting a delete or rejecting an insert).
	 * 
	...
	/**
	 * @property {String} [dropTrackingPolicy="insert"]
	 * @since FLITE 1.7.06
	 * Legal values:
	 * 
	 * - `"none"`: no tracking after a drop event
	 * - `"insert"`: Track only insertions due to a drop event, ignore deleted content
